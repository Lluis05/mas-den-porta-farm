...
type Props = {
  /** Si ve, el formulari s'omple amb aquests valors (mode edició). */
  inicial?: CicleEditable | null;
  textBoto: string;
  onDesa: (dades: NouCicle) => Promise<void>;
};
...
/**
 * El formulari d'un cicle d'engreix, compartit entre crear-ne un de nou i
 * editar-ne un d'existent. Així les dues pantalles no poden divergir.
 */
export function FormulariCicle({ inicial, textBoto, onDesa }: Props) {
  ...
  // En mode edició, omplim el formulari un sol cop, quan ja tenim les sales.
  const jaOmplert = useRef(false);
  useEffect(() => {
    if (jaOmplert.current || !inicial || sales.length === 0) return;
    jaOmplert.current = true;

    setBandaId(inicial.banda_id);
    setData(new Date(`${inicial.data_entrada}T12:00:00`));

    const perSala = new Map<string, { porcs: number; corrals: Set<string> }>();
    for (const c of inicial.corrals) {
      const actual = perSala.get(c.sala_id) ?? { porcs: 0, corrals: new Set() };
      actual.porcs += c.porcs_entrada;
      actual.corrals.add(c.corral_id);
      perSala.set(c.sala_id, actual);
    }
    ...
    setTriades(
      [...perSala.entries()]
        .map(([salaId, v]) => ({
          salaId,
          numero: sales.find((s) => s.id === salaId)?.numero ?? 0,
          porcs: String(v.porcs),
          seleccionats: v.corrals,
        }))
  ...
  return (
    ...
    >
      ...
      >
          ...
          return (
            <View key={t.salaId} style={styles.targeta}>
              ...
              <GraellaCorrals
                ...
                ocupats={inicial ? new Set() : ocupats}
        ...
        >
          <Text style={styles.botoText}>{desant ? 'Desant…' : textBoto}</Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoidingView>
  );
